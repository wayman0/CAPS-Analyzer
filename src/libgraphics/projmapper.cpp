...
int PROJMapper::makeMap(dataMap *map, FILETYPE dataTypes, association* dataClasses) {
  if (!dataClasses)
    return 0;
...
int PROJMapper::makeMapHealpix(dataMap *map, FILETYPE dataTypes, association* dataClasses) {
  ...
  m_projection = proj_create(m_threadingContext,m_projString.c_str());
  if (m_projection == 0)
    return -1;

  m_geodetic = proj_crs_get_geodetic_crs(m_threadingContext,m_projection);
  PJ_AREA *area = NULL;
  const char *const *options = NULL;
  m_projTransform = proj_create_crs_to_crs_from_pj(m_threadingContext,m_geodetic,m_projection,area,options);
