...
//  connect(ui->resEdit, &QLineEdit::editingFinished, [=](){healpixDialog::syncResolution();});
  ...
  ui->minMaxScale->setChecked(false);
  ui->pixelAvg->setChecked(false);
  ui->pixelDev->setChecked(false);
  ui->pixelVar->setChecked(false);
...
//  dataAssoc->addEngine(dataEngines::Pixelization, HealPIX);
  ...
  if (isConfigured) {
    ...
    if (usePixAvg)
      ui->pixelAvg->setChecked(true);
    else
      ui->pixelAvg->setChecked(false);

    if (usePixDev)
      ui->pixelDev->setChecked(true);
    else
      ui->pixelDev->setChecked(false);

    if (usePixVar)
      ui->pixelVar->setChecked(true);
    else
      ui->pixelVar->setChecked(false);

    if (doAvgNorm)
      ui->avgNormalize->setChecked(true);
    else
      ui->avgNormalize->setChecked(false);

    if (doVarNorm)
      ui->varNormalize->setChecked(true);
    else
      ui->varNormalize->setChecked(false);

    if (doMinMax)
      ui->minMaxScale->setChecked(true);
    else
      ui->minMaxScale->setChecked(false);
...
void healpixDialog::validate() {
  ...
  bool oldUsePixAvg = usePixAvg;
  usePixAvg = ui->pixelAvg->isChecked();
  if (usePixAvg != oldUsePixAvg)
    dirty = true;

  bool oldUsePixDev = usePixDev;
  usePixDev = ui->pixelDev->isChecked();
  if (usePixDev != oldUsePixDev)
    dirty = true;

  bool oldUsePixVar = usePixVar;
  usePixVar = ui->pixelVar->isChecked();
  if (usePixVar != oldUsePixVar)
    dirty = true;

  bool oldDoAvgNorm = doAvgNorm;
  doAvgNorm = ui->avgNormalize->isChecked();
  if (doAvgNorm != oldDoAvgNorm)
    dirty = true;

  bool oldDoVarNorm = doVarNorm;
  doVarNorm = ui->varNormalize->isChecked();
  if (doVarNorm != oldDoVarNorm)
    dirty = true;

  bool oldDoMinMax = doMinMax;
  doMinMax = ui->minMaxScale->isChecked();
  if (doMinMax != oldDoMinMax)
    dirty = true;
