...
void mainWindow::openFile()
{
  ...
  while (dataTypes != fileType::FILETYPE_LIMIT)
  {
    ...
    /*
     * FILETYPE[] types = filemanager.getDAtaTypes()
     * for(filetype of types)
     *  dataSelectionGUI.add(tyeps)
     *
     * FILETYPE types2[]
     * for(checkbox of dataSelectionGUI)
     *  types2.add(checkbox.name);
     *
     * open(types2);
     */
  ...
  if (openDialog->exec()) {
    ...
    FILETYPE* dataTypes = 0;
    try
    {
      dataTypes = s_association->fileIOEngine()->open(numTypes);
    }
    catch (ERRORCODES error) {
...
void mainWindow::saveFile() {
  ...
  if (saveDialog->exec()) {
    ...
    /*
     * for(datatypes)
     * if(association.exists(datatypes))
     * dataSelection.add(dataTypes); // dataselection is a window with checkboxes
     *
     *display dataSelection
     *
     * selectedData[]
     *for(checkbox of dataSelection)
     *  selectedData.add(checkbox.name)
     *
     * save(selectedData);
    */

    // get data type
    selectedDataType = fileType::InputData;
