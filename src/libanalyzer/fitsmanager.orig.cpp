...
matrixData<double> *fitsManager::getMatrixD() {
  ...
  try {
    LOAD("hduName", fitsHDUName);
    LOAD("TFIELDS", dataCols);
    LOAD("NAXIS2", dataRows);
  ...
  fitsData.resize(dataCols * dataRows);
  try{
    m_ptr->pHDU().read(fitsData);
  }
  catch (CCfits::FitsException& err) {
  ...
  for(int row = 0; row < dataRows; row += 1)
  {
    for(int col = 0; col < dataCols; col += 1)
    {
      int index = col + (row * dataCols);
      (*d_mat)[col][row] = fitsData[index];

      //if(fitsData[index] != 10)
        //std::cout << fitsData[index] << " index " << index << " row: " << row << " col " << col << std::endl;
    }
  }
...
//      informProgress(currOp / updateUnit);
    ...
    */
    ...
    fitsData.resize(dataCols * dataRows * dataSlices * 2 + 1);
    m_ptr->pHDU().read(fitsData);
    int imagOffset = dataCols * dataRows * dataSlices;
    std::cout << imagOffset;
...
//      informProgress(currOp / updateUnit);
  ...
  try {
    STORE("hduName", hduName, "Data Processing Step");
    STORE("TFIELDS", fitsData.size(), "Size of Data");
 ...
 // hduName = "PIXEL_OCCUPANCY";
  try {
   ...
   m_ptr->pHDU().write(1, fitsData.size(), fitsData);
  }
  catch (CCfits::FitsException& err) {
    snprintf(fits_err,FITS_ERR_LEN,"%s",err.message().c_str());
    m_err = fileFitsError;
...
//  int nSides = 0;
  try {
    STORE("hduName", hduName, "Data Processing Step");
    STORE("TFIELDS", fitsData.size(), "Data Size");
...
bool fitsManager::saveMatrixD(matrixData<double> *m) {
  ...
  try {
    STORE("hduName", hduName, "Data Processing Step");
    STORE("TFIELDS", m_cols, "Data Columns");
    STORE("NAXIS2",  m_rows, "Data Rows");
  ...
  for(int r = 0; r < m_rows; r += 1)
  {
      ...
      {
          int index = c + (r * m_cols);
          fitsData[index] = (*m)[c][r];

          //if(fitsData[index] != 10)
            //std::cout << fitsData[index] << " index " << index << " row " << r << " col " << c << "\n";
      }
  }
...
bool fitsManager::saveCubeCD(cubeData<complex<double> > *c) {
  ...
  try {
    ...
    STORE("TFIELDS", m_cols,  "Data Columns");
    STORE("NAXIS2",  m_rows,  "Data Rows");
    STORE("NAXIS3",  m_slices,"Data Slices");
...
//          informProgress(currOp / updateUnit);
  ...
  //binTable->column(colName[col]).write(fitsData,1);
    // write the imaginary part of the data after the real part of the data
    m_ptr->pHDU().write(1,                       fitsDataReal.size(), fitsDataReal);
    m_ptr->pHDU().write(1 + fitsDataReal.size(), fitsDataImag.size(), fitsDataImag);
  }
  catch (CCfits::FitsException& err)
  {
    snprintf(fits_err,FITS_ERR_LEN,"%s",err.message().c_str());
