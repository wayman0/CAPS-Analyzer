...
long inputMatrixData::create(int l, int m, double value) {
  ...
  thetaInc = m_resDec * (M_PI / 180.0); // convert the spacing inbetween horizontal lines to radians
  phiInc = m_resRA * (M_PI / 180.0); // convert the spacing inbetween vertical lines to radians
  numOps =  m_rows * m_cols;
  updateUnit = numOps/100;
  if(updateUnit < 1)
    updateUnit = 1;
  ...
  while(theta < M_PI) {
    ...
    //convTheta = theta * toDegrees;

    phi = 0;
    while(phi < twoPi) {
      convPhi = (phi * toDegrees) - 180.0;
      //convPhi = phi * toDegrees;

      //angle2bin(convTheta,convPhi,x,y);
      angle2bin((M_PI / 2) - theta,phi - M_PI, x, y, false); // convert theta and phi to an x and y
