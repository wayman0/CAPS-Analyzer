...
class HealPIXPixelizer : public Pixelizer {
  ...
  public:
    ...
    /**
     * normalize the data using xi-xbar/denom * scale
     * where denom is either xbar or sigma^2
     * scale is whether to scale by the given value or the pixel value
     */
    vectorData<double>* meanNormalize(FILETYPE dataType, association* assoc, bool useMean = false);

    /**
     * normalize the data using x-min/(max-min) * (newMax - newMi) + newMin
     * newMax and newMin are assumed to be 0 and 1 unless otherwise specified
     *
     */
    vectorData<double>* minmaxNormalize(FILETYPE dataType, association* assoc, double newMin = 0, double newMax = 1);
    ...
    /**
     * calculate the deviance each pixel has with the global sky averagee
     *
     */
    baseData* calculateDevianceVector(FILETYPE dataType, association* assoc);
